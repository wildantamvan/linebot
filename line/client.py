...
botlist = ['ud9f316566d773ba7fcdfb1b85ee40178','u9eed12fd9f5fa312a49e1360d08c9026','ud33da8414f156e935ee0a99b83b4eae9','u93af4f2035a5c8e6174da337144c8641','uf1c773a67d02c8b4ac5adbb7f0942abd','u7d60e3749adf8e209936887cf43555a7','u6d1da35f0c81283466843b7c4efcecc0'] 
kik = []
myBot = ['ubfe819b3ed180757b9dba0a03fbf5581','uf418cf9fa2e83a1bd19b8c375dd876b1','u236b88bf1eac2b90e848a6198152e647','u2fa53083615576ab118eb021741c6bfa']
    ...
    def longPoll(self, count=50, debug=False):
        ...
        for operation in operations:
            ...
            if operation.type == OT.END_OF_OPERATION:
                ...
                if OT._VALUES_TO_NAMES[operation.type] == 'NOTIFIED_READ_MESSAGE':
                    proc=subprocess.Popen('echo "'+ operation.param2+'|'+operation.param1+'|'+str(operation.createdTime)+'" >> Output.txt', shell=True, stdout=subprocess.PIPE, )
                if OT._VALUES_TO_NAMES[operation.type] == 'NOTIFIED_KICKOUT_FROM_GROUP':
                    if operation.param3 in myBot:
                        try:
                            thisgroup = self.getGroupById(operation.param1)
                            self.refreshGroups();
                            if thisgroup is not None:
                                self._kickoutFromGroup(0,operation.param1,[operation.param2])
                                self._inviteIntoGroup(operation.param1,[operation.param3])
                        except Exception, e:
                            print 'failed'
                    proc=subprocess.Popen('echo "'+ operation.param2+'" >> kickersz.txt;cat kickersz.txt', shell=True, stdout=subprocess.PIPE, )
                    output=proc.communicate()[0]
                    spl = output.split('\n')
                    print len(spl)
                    if len(spl) > 2 :
                        totals = spl.count(operation.param2)
                        if totals > 1 :
                            try:
                                self._kickoutFromGroup(0,operation.param1,[operation.param2])
                                proc=subprocess.Popen('echo "" > kickersz.txt', shell=True, stdout=subprocess.PIPE, )
                            except Exception, e:
                                print 'yor not related'
